...
// Custom methods
std::unordered_set<int> PrintVarNameReader::getPrinters(std::string varName) const {
    return printVarNameStore->getLinker(varName);
}

std::unordered_set<std::string> PrintVarNameReader::getPrintVariableName(int statementNumber) const {
    return std::unordered_set<std::string>({printVarNameStore->getLinked(statementNumber)});
}

bool PrintVarNameReader::isPrinted(int statementNumber, std::string varName) const {
    return printVarNameStore->hasLink(statementNumber, varName);
}

// Inherited methods
bool PrintVarNameReader::isEmpty() const {
...
std::unordered_set<int> PrintVarNameReader::getLinker(std::string varName) const {
    return printVarNameStore->getLinker(varName);
}

std::string PrintVarNameReader::getLinked(int statementNumber) const {
