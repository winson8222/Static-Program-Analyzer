 ...
 */
std::shared_ptr<ResultTable> WithStrategy::evaluateQuery(PKBReaderManager& pkbReaderManager,
	const ParsingResult& parsingResult, const Clause& clause, const std::shared_ptr<ResultTable>& originalResultTable) {
	...
	if (isInteger(firstParam.getValue()) && isInteger(secondParam.getValue())) {
		...
		if (rows1.size() != 0 && rows2.size() != 0) {
			...
			for (std::pair<std::string, std::string> row : rows2) {
				insertSingleColRowToTable(row, resultTable);
			}
		}
		else if (rows1.size() != 0) {
			for (std::pair<std::string, std::string> row : rows1) {
				insertSingleColRowToTable(row, resultTable);
			}
		}
...
std::unordered_set<std::string> WithStrategy::processParam(Token param, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable) {
	// check if param.getValue() is an integer or a quoted std::string
	...
	if (isInteger(param.getValue())) {
		//return std::vector of param.getValue()
		...
		std::pair<std::string, std::string> attributes = extractAttributes(param, pkbReaderManager);
 ...
 */
std::pair<std::string, std::string> WithStrategy::extractAttributes(Token param, PKBReaderManager& pkbReaderManager)
{
...
std::vector<std::string> WithStrategy::mapStringSetToIntSet(PKBReaderManager& pkbReaderManager, const std::vector<std::string>& stringSet, std::string& synonymType)
{
	...
	if (synonymType == "read")
	{
		...
		{
			std::unordered_set<int> linkedStmts = readVarNameReader->getLinker(varName);
	...
	else if (synonymType == "print")
	{
		...
		for (std::string varName : stringSet)
		{
			std::unordered_set<int> linkedStmts = printVarNameReader->getLinker(varName);
			intSet.insert(intSet.end(), linkedStmts.begin(), linkedStmts.end());
		}
	}
	else if (synonymType == "call")
	{
		...
		{
			std::unordered_set<int> linkedStmts = callProcNameReader->getLinker(procName);
			intSet.insert(intSet.end(), linkedStmts.begin(), linkedStmts.end());
		}
	}
	std::vector<std::string> mappedIntSet;
	for (int i : intSet) {
...
std::vector<std::pair<std::string, std::string>> WithStrategy::populateResultTable(
	...
	const std::vector<std::string>& intersection, Token param, PKBReaderManager& pkbReaderManager)
{
	...
	if (!isQuotedString(param.getValue()) && !isInteger(param.getValue())) {
		...
		if (isIntegerStored(synonymType, attribute)) {
			std::vector<std::string> mappedIntersection = mapStringSetToIntSet(pkbReaderManager, intersection, synonymType);
			// insert column with value of synonym and rows with the intersection of the two sets
			for (std::string elem : mappedIntersection) {
				rows.push_back(std::make_pair(synonym, elem));
			}
		}
		else {
