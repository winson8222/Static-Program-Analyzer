...
public:
	...
	std::pair<std::string, std::string> extractAttributes(Token param);
	...
	const std::map<std::string, std::function<std::unordered_set<std::string>(std::string, PKBReaderManager&, const std::shared_ptr<ResultTable>&)>> attrToAllEntitiesMap = {
		{
			"stmt.stmt#", [this](std::string synonym, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable) {
		...
		{
			"read.varName", [this](std::string synonym, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable) {
				std::shared_ptr<ReadVarNameReader> readerVarNameReader = pkbReaderManager.getReadVarNameReader();
				return retrieveIntStringLinks(synonym, resultTable, readerVarNameReader);
			}
		},
		{
			"read.stmt#", [this](std::string synonym, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable) {
				 std::shared_ptr<ReadReader> readReader = pkbReaderManager.getReadReader();
				 return retrieveIntEntities(synonym, resultTable, readReader);
			}
		},
		{
			"print.varName", [this](std::string synonym, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable) {
				std::shared_ptr<PrintVarNameReader> printVarNameReader = pkbReaderManager.getPrintVarNameReader();
				return retrieveIntStringLinks(synonym, resultTable, printVarNameReader);
			}
		},
		{
			"print.stmt#", [this](std::string synonym, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable) {
				std::shared_ptr<PrintReader> printReader = pkbReaderManager.getPrintReader();
	...
	void populateTwoColumns(std::string commonAttrValue, std::string firstParam, std::string firstParamAttrType,
		std::string secondParam, std::string secondParamAttrType, PKBReaderManager& pkbReaderManager, const std::shared_ptr<ResultTable>& resultTable);
};
