...
TEST_CASE("stmt s1, s2; variable v; Select BOOLEAN such that not Follows(1, 2)") {
    ...
    string query = "stmt s1, s2; variable v; Select BOOLEAN such that not Follows(1, 2)";
...
TEST_CASE("stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2)") {
    ...
    string query = "stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2) and Follows(s1, 3)") {
    ...
    string query = "stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2) and Follows(s1, 3)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2) and Follows(s2, 3)") {
    ...
    string query = "stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2) and Follows(s2, 3)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
string query = "stmt s1, s2; variable v; Select s1 such that not Follows(s1, 2) and Follows(s1, s2)";
Tokenizer tokenizer(query);
auto tokens = tokenizer.tokenize();
QueryParser parser(tokens);
auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select <s1, s2> such that not Follows(s1, s2)") {
    ...
    string query = "stmt s1, s2; variable v; Select <s1, s2> such that not Follows(s1, s2)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
    QueryEvaluator evaluator(pkbReaderManager, parsingResult);
    std::unordered_set<string> res = evaluator.evaluateQuery();
    REQUIRE(res == std::unordered_set<string>{ "1 3", "2 2", "3 3", "1 1", "2 1", "3 2", "3 1" });
}
...
TEST_CASE("stmt s1, s2; variable v; Select s1 such that Follows(s2, 3) and not Follows(s1, 2)") {
    ...
    string query = "stmt s1, s2; variable v; Select s1 such that Follows(s2, 3) and not Follows(s1, 2)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select <s1, s2> such that not Follows(s1, s2) and Follows(s1, 1)") {
    ...
    string query = "stmt s1, s2; variable v; Select <s1, s2> such that not Follows(s1, s2) and Follows(s1, 1)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select <s1, s2> such that not Follows(s1, 3) and Follows(s1, 2)") {
    ...
    string query = "stmt s1, s2; variable v; Select <s1, s2> such that not Follows(s1, 3) and Follows(s1, 2)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
    QueryEvaluator evaluator(pkbReaderManager, parsingResult);
    std::unordered_set<string> res = evaluator.evaluateQuery();
    REQUIRE(res == std::unordered_set<string>{ "1 1", "1 2", "1 3" });
}
// not between 2 ands
TEST_CASE("stmt s1, s2; variable v; Select s1 such that Follows(s1, s2) and not Follows(s1, 5) and Parent(1, 2)") {
    ...
    string query = "stmt s1, s2; variable v; Select s1 such that Follows(s1, s2) and not Follows(s1, 5) and Parent(1, 2)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select s1 such that Uses(s1, v) and not Follows(s1, 3) and Parent(1, 2)") {
    ...
    string query = "stmt s1, s2; variable v; Select s1 such that Uses(s1, v) and not Follows(s1, 3) and Parent(1, 2)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
...
TEST_CASE("stmt s1, s2; variable v; Select <v, s1, s2> such that Uses(s1, v) and not Uses(s2, v)") {
    ...
    string query = "stmt s1, s2; variable v; Select <v, s1, s2> such that Uses(s1, v) and not Uses(s2, v)";
    Tokenizer tokenizer(query);
    auto tokens = tokenizer.tokenize();
    QueryParser parser(tokens);
    auto parsingResult = parser.parse();
