...
TEST_CASE("src/qps/evaluator/suchThatStrategies/ModifiesStrategy/4") {
	...
	QueryEvaluator evaluator(pkbReaderManager, pkbCacheManager, parsingResult);
	std::unordered_set<std::string> res = evaluator.evaluateQuery();
	REQUIRE((res == std::unordered_set<std::string>{ "1", "2", "3" } || res == std::unordered_set<std::string>{"2", "3", "1"} || res == std::unordered_set<std::string>{"3", "2", "1"}));
}
...
TEST_CASE("src/qps/evaluator/suchThatStrategies/ModifiesStrategy/6") {
	...
	QueryEvaluator evaluator(pkbReaderManager, pkbCacheManager, parsingResult);
	std::unordered_set<std::string> res = evaluator.evaluateQuery();
	//    REQUIRE(res == std::unordered_set<std::string>{ "y", "1 2 3 4", "2 3 4" });
}
...
TEST_CASE("src/qps/evaluator/suchThatStrategies/ModifiesStrategy/7") {
	...
	QueryEvaluator evaluator(pkbReaderManager, pkbCacheManager, parsingResult);
	std::unordered_set<std::string> res = evaluator.evaluateQuery();
	REQUIRE(res == std::unordered_set<std::string>{"y 2 x", "y 3 x", "y 4 x", "y 4 y", "y 2 y", "y 2 z", "y 3 z", "y 3 y", "y 4 z"});
}
...
TEST_CASE("src/qps/evaluator/suchThatStrategies/ModifiesStrategy/9") {
	std::shared_ptr<PKBManager> pkbManager = std::make_shared<PKBManager>();
	std::shared_ptr<PKBReaderManager> pkbReaderManager = pkbManager->getPKBReaderManager();
    std::shared_ptr<PKBCacheManager> pkbCacheManager = pkbManager->getPKBCacheManager();
	std::shared_ptr<PKBWriterManager> pkbWriterManager = pkbManager->getPKBWriterManager();
...
TEST_CASE("src/qps/evaluator/suchThatStrategies/ModifiesStrategy/10") {
	std::shared_ptr<PKBManager> pkbManager = std::make_shared<PKBManager>();
	std::shared_ptr<PKBReaderManager> pkbReaderManager = pkbManager->getPKBReaderManager();
    std::shared_ptr<PKBCacheManager> pkbCacheManager = pkbManager->getPKBCacheManager();
	std::shared_ptr<PKBWriterManager> pkbWriterManager = pkbManager->getPKBWriterManager();
