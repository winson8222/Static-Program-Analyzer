...
TEST_CASE("sp/SourceProcessor: Parent(unit)") {
    ...
    SECTION("Basic Parent") {
        SECTION("Test Parent") {
			...
			ParentExtractor parentExtractor1(pkbWriterManager->getParentWriter());
			parentExtractor1.extract(ast1, ast2);
		...
		SECTION("Test ParentT") {
			...
			ParentTExtractor parentExtractor2(pkbWriterManager->getParentTWriter());
			parentExtractor2.extract(ast3, ast4);
